...
const opCenterDropdown = document.getElementById('op-centerDropdown-s5');
const ipVillageDropdown = document.getElementById('ip-villageDropdown-s5');
const opVillageDropdown = document.getElementById('op-villageDropdown-s5');
...
function updateDatatable(selectedDate) {
    ...
    fetch(`http://isdom.online/dash_board/tickets/all?siteNo=5&selectedDate=${selectedDate}`)
...
const s5_in_grph = c3.generate(ipChartOptions);
    ...
    if (centerId !== "") {
        fetch(`http://isdom.online/dash_board/villages?centerId=${centerId}`)
            ...
            .then(data => {
                ...
                document.getElementById("op-villageDropdown-s5").style.display = "block";
...
const s5_out_grph = c3.generate(opChartOptions);

function updateOutputGraph_s5(isVillage) {
    ...
    const url1 = `http://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=وقود بديل&siteNo=5&startDate=${startDatex}&endDate=${endDatex}&centerId=${selectedCenter}&villageId=${selectedVillage}`; // وقود بديل
    const url2 = `http://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=اسمدة عضوية&siteNo=5&startDate=${startDatex}&endDate=${endDatex}&centerId=${selectedCenter}&villageId=${selectedVillage}`; // اسمدة عضوية
    const url3 = `http://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=مرفوضات&siteNo=5&startDate=${startDatex}&endDate=${endDatex}&centerId=${selectedCenter}&villageId=${selectedVillage}`; //مرفوضات
    const url4 = `http://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=مفروزات&siteNo=5&startDate=${startDatex}&endDate=${endDatex}&centerId=${selectedCenter}&villageId=${selectedVillage}`; //مفروزات
...
const s5_ip_chart = new Chart(document.getElementById('s5-ip-chart'), {
...
function updateIPChartData(startDate, endDate) {
    ...
    Promise.all([
        ...
        .then(([dataset1Data, dataset2Data]) => {
            ...
            s5_ip_chart.data.datasets[0].data[0] = dataset1Value || 0;
            s5_ip_chart.data.datasets[0].data[1] = dataset2Value || 0;
...
const s5_op_chart = new Chart(document.getElementById('s5-op-chart'), {
    type: 'doughnut',
...
function updateOPChartData(startDate, endDate) {
    ...
    const urlOP3 = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مرفوضات&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مرفوضات
    ...
    Promise.all([
        ...
        .then(([category1Data, category2Data, category3Data, category4Data]) => {
            // Update the dataset values in the chart
            s5_op_chart.data.datasets[0].data[0] = category1Data || 0;
            s5_op_chart.data.datasets[0].data[1] = category2Data || 0;
            s5_op_chart.data.datasets[0].data[2] = category3Data || 0;
            s5_op_chart.data.datasets[0].data[3] = category4Data || 0;
...
function updateRejBox(startDate, endDate) {
    const urlAcc = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات  تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    const urlRej = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مرفوضات&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مرفوضات
    ...
    Promise.all([
        ...
        .then(([accData, rejData]) => {
            ...
            if (isNaN(percentage))
                document.getElementById("s5-accepted").textContent = 0 + "%"
            else {
                document.getElementById("s5-rejected-per").textContent = percentage.toFixed(1) + "%"
                document.getElementById("s5-accepted").textContent = "من " + accData + " طن";

                const progressBar = document.getElementById("s5-rej-progress");
...
function updateAccBox(startDate, endDate) {
    ...
    Promise.all([
        ...
        .then(([accData, totInData]) => {
            ...
            if (isNaN(percentage))
                document.getElementById("s5-accepted-per").textContent = 0 + "%"
            else {
                document.getElementById("s5-accepted-per").textContent = percentage.toFixed(1) + "%"

                const progressBar = document.getElementById("s5-acc-progress");
                updateProgressBar(progressBar, percentage);
            }
        })
        .catch(error => {
...
function updateOPBox(startDate, endDate) {
    const url = `http://isdom.online/dash_board/tickets/itemType/weight?itemType=مخرجات&siteNo=5&startDate=${startDate}&endDate=${endDate}`;
...
function updateMassBox(startDate, endDate) {
    const totInURL = `http://isdom.online/dash_board/tickets/itemType/weight?itemType=مدخلات&siteNo=5&startDate=${startDate}&endDate=${endDate}`;
    const totOutURL = `http://isdom.online/dash_board/tickets/itemType/weight?itemType=مخرجات&siteNo=5&startDate=${startDate}&endDate=${endDate}`;
    const accURL = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات  تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    const rejURL = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات لا تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات لا تصلح للمعالجة
    ...
    Promise.all([
        ...
        document.getElementById("s5-mass-box").textContent = newValue.toFixed(0) + " طن";
    })
        .catch(error => {
            console.error('Error:', error);
        });
