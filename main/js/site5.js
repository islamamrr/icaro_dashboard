...
const ipCenterDropdown = document.getElementById('ip-centerDropdown-s5');
const ipVillageDropdown = document.getElementById('ip-villageDropdown-s5');
...
function updateDatatable(startDate, endDate) {
    ...
    const tbody = document.querySelector('#site5_table tbody');


    fetch(`https://isdom.online/dash_board/tickets/all?siteNo=5&startDate=${startDate}&endDate=${endDate}`)
        .then(response => response.json())
        .then(data => {

            dataTableJSONData = data;

            $('#site5_table').DataTable().destroy();
            tbody.innerHTML = '';
                    ...
                    if (userRole === "Admin")
                        if (key === 'carTwoDate' || key === 'carTwoTime' || key === 'secondWeight') {
                            ...
                            cell.addEventListener('keyup', function () {
                                ...
                                fetch(`https://isdom.online/dash_board/tickets/${rowData.ticketId}/5`, {
                                    method: 'PUT',
                                    headers: {
...
//Get list of centers
function getCenters() {
    fetch('https://isdom.online/dash_board/centers')
        .then(response => response.json())
        .then(data => {
...
// function getOpNames() {
//     fetch('https://isdom.online/dash_board/items/items/مخرجات/item-names')
//         .then(response => response.json())
//         .then(data => {
//         })
//         .catch(error => {
...
function updateIpVillageDropdown(centerId) {
    if (centerId !== "") {
        fetch(`https://isdom.online/dash_board/villages?centerId=${centerId}`)
            .then(response => response.json())
            .then(data => {

                document.getElementById("ip-villageDropdown-s5").style.display = "block";
...
function updateInputGraph_s5(isVillage) {
    ...
    const url1 = `https://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=مخلفات  تصلح للمعالجة&siteNo=5&startDate=${startDateIPGraph}&endDate=${endDateIPGraph}&centerId=${selectedCenter}&villageId=${selectedVillage}`; //مخلفات تصلح للمعالجة
    const url2 = `https://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=مخلفات لا تصلح للمعالجة&siteNo=5&startDate=${startDateIPGraph}&endDate=${endDateIPGraph}&centerId=${selectedCenter}&villageId=${selectedVillage}`; //مخلفات لا تصلح للمعالجة
    ...
    Promise.all([
        ...
        s5_in_grph.data.datasets[0].data = values1 || 0;
        s5_in_grph.data.datasets[1].data = values2 || 0;
        s5_in_grph.data.labels = categories || 0;
...
function updateOutputGraph_s5() {
    const waqoodURL = `https://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=وقود بديل&siteNo=5&startDate=${startDateOPGraph}&endDate=${endDateOPGraph}`; // وقود بديل
    const asmedaURL = `https://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=اسمدة عضوية&siteNo=5&startDate=${startDateOPGraph}&endDate=${endDateOPGraph}`; // اسمدة عضوية
    const marfoodatURL = `https://isdom.online/dash_board/tickets/itemName-site/weight-date-list?siteNo=3&clientType=${currentClientType}&startDate=${startDateOPGraph}&endDate=${endDateOPGraph}`; // مرفوضات
    const mafroozatURL = `https://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=مفروزات&siteNo=5&startDate=${startDateOPGraph}&endDate=${endDateOPGraph}`; //مفروزات
    Promise.all([
        ...
        s5_out_grph.data.datasets[0].data = valuesWaqood || 0;
        s5_out_grph.data.datasets[1].data = valuesAsmeda || 0;
        s5_out_grph.data.datasets[2].data = valuesMarfoodat || 0;
        s5_out_grph.data.datasets[3].data = valuesMafroozat || 0;
        s5_out_grph.data.labels = categories || 0;
...
const s5_ip_chart = c3.generate(initialIPChartData);

function updateIPChartData(startDate, endDate) {
    const urlIP1 = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات  تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    const urlIP2 = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات لا تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات لا تصلح للمعالجة
    Promise.all([
...
const s5_op_chart = c3.generate(initialOPChartData);


function updateOPChartData(startDate, endDate) {
    const urlOP1 = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=اسمدة عضوية&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // اسمدة عضوية
    const urlOP2 = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=وقود بديل&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // وقود بديل
    const urlOP3 = `https://isdom.online/dash_board/tickets/itemName/weight?siteNo=3&clientType=${currentClientType}&startDate=${startDate}&endDate=${endDate}`; // مرفوضات
    const urlOP4 = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مفروزات&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مفروزات
...
function updateIPBox(startDate, endDate) {
    const url = `https://isdom.online/dash_board/tickets/itemType/weight?itemType=مدخلات&siteNo=5&startDate=${startDate}&endDate=${endDate}`;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            document.getElementById("s5-ip-box").textContent = newValue + " طن";
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
function updateAccIPBox(startDate, endDate) {
    const url = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات  تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    fetch(url)
        .then(response => response.json()).catch(() => 0)
        .then(data => {
            const newValue = data;
            document.getElementById("s5-accepted-ip-box").textContent = newValue + " طن";
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
function updateRejIPBox(startDate, endDate) {
    const url = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات لا تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    fetch(url)
        .then(response => response.json()).catch(() => 0)
        .then(data => {
            const newValue = data;
            document.getElementById("s5-rejected-ip-box").textContent = newValue + " طن";
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
function updateRejRateBox(startDate, endDate) {
    const accUrl = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات  تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    const rejUrl = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات لا تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`;
    const marfoodatURL = `https://isdom.online/dash_board/tickets/itemName/weight?siteNo=3&clientType=${currentClientType}&startDate=${startDate}&endDate=${endDate}`; // مرفوضات
...
function updateAccRateBox(startDate, endDate) {
    const accUrl = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات  تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    const totInUrl = `https://isdom.online/dash_board/tickets/itemType/weight?itemType=مدخلات&siteNo=5&startDate=${startDate}&endDate=${endDate}`;
...
function updateInOperationBox() {
    ...
    const percentagesURL = 'https://isdom.online/dash_board/accumulated/percentage?siteNo=5';
    const accumulatedWeightsURL = 'https://isdom.online/dash_board/accumulated/weight?siteNo=5'
    const acceptedInputURL = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات  تصلح للمعالجة&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    const asmedaURL = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=اسمدة عضوية&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // اسمدة عضوية
    const waqoodURL = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=وقود بديل&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // وقود بديل
    const marfoodatURL = `https://isdom.online/dash_board/tickets/itemName/weight?siteNo=3&clientType=${currentClientType}&startDate=${startDate}&endDate=${endDate}`; // مرفوضات
    const mafroozatURL = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مفروزات&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مفروزات
...
function updateOPBox(startDate, endDate) {
    const url = `https://isdom.online/dash_board/tickets/output/weight?itemType=مخرجات&siteNo=5&clientType=${currentClientType}&startDate=${startDate}&endDate=${endDate}`;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            document.getElementById("s5-op-box").textContent = newValue + " طن";
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
function updateAsmedaOPBox(startDate, endDate) {
    const url = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=اسمدة عضوية&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    fetch(url)
        .then(response => response.json()).catch(() => 0)
        .then(data => {
            const newValue = data;
            document.getElementById("s5-asmeda-op-box").textContent = newValue + " طن";
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
function updateWaqoodOPBox(startDate, endDate) {
    const url = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=وقود بديل&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    fetch(url)
        .then(response => response.json()).catch(() => 0)
        .then(data => {
            const newValue = data;
            document.getElementById("s5-waqood-op-box").textContent = newValue + " طن";
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
function updateMarfoodatOPBox(startDate, endDate) {
    const url = `https://isdom.online/dash_board/tickets/itemName/weight?siteNo=3&clientType=${currentClientType}&startDate=${startDate}&endDate=${endDate}`;
    fetch(url)
        .then(response => response.json()).catch(() => 0)
        .then(data => {
            const newValue = data;
            document.getElementById("s5-marfoodat-op-box").textContent = newValue + " طن";
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
function updateMafroozatOPBox(startDate, endDate) {
    const url = `https://isdom.online/dash_board/tickets/itemName/weight?itemName=مفروزات&siteNo=5&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    fetch(url)
        .then(response => response.json()).catch(() => 0)
        .then(data => {
            const newValue = data;
            document.getElementById("s5-mafroozat-op-box").textContent = newValue + " طن";
        })
        .catch(error => {
            console.error('Error:', error);
        });
...
$(document).ready(function () {
    ...
    getCenters();
    updateInputGraph_s5(false);
    updateOutputGraph_s5();
...
document.getElementById('openOperationPopupBtn').addEventListener('click', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        const urlPercentage = 'https://isdom.online/dash_board/accumulated/update-percentage/5'
        const urlAccuWeight = 'https://isdom.online/dash_board/accumulated/update-weight/5'
