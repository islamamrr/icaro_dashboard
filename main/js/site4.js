...
function updateDatatable(selectedDate) {
    ...
    fetch(`http://isdom.online/dash_board/tickets/all?siteNo=4&selectedDate=${selectedDate}`)
...
const s4_in_grph = c3.generate(ipChartOptions);
...
function updateInputGraph_s4(isVillage) {
    ...
    const url1 = `http://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=مخلفات  تصلح للمعالجة&siteNo=4&startDate=${startDatex}&endDate=${endDatex}&centerId=${selectedCenter}&villageId=${selectedVillage}`; //مخلفات تصلح للمعالجة
    const url2 = `http://isdom.online/dash_board/tickets/itemName-site/weight-date-list?itemName=مخلفات لا تصلح للمعالجة&siteNo=4&startDate=${startDatex}&endDate=${endDatex}&centerId=${selectedCenter}&villageId=${selectedVillage}`; //مخلفات لا تصلح للمعالجة
...
function updateIPChartData(startDate, endDate) {
    const urlIP1 = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات  تصلح للمعالجة&siteNo=4&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    const urlIP2 = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات لا تصلح للمعالجة&siteNo=4&startDate=${startDate}&endDate=${endDate}`; // مخلفات لا تصلح للمعالجة
    Promise.all([
        ...
        .then(([dataset1Data, dataset2Data]) => {
            ...
            s4_ip_chart.data.datasets[0].data[0] = dataset1Value || 0;
            s4_ip_chart.data.datasets[0].data[1] = dataset2Value || 0;
...
function updateOPChartData(startDate, endDate) {
    const urlOP1 = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=اسمدة عضوية&siteNo=4&startDate=${startDate}&endDate=${endDate}`; // اسمدة عضوية
    const urlOP2 = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=وقود بديل&siteNo=4&startDate=${startDate}&endDate=${endDate}`; // وقود بديل
    const urlOP3 = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مرفوضات&siteNo=4&startDate=${startDate}&endDate=${endDate}`; // مرفوضات
    const urlOP4 = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مفروزات&siteNo=4&startDate=${startDate}&endDate=${endDate}`; // مفروزات
    ...
    Promise.all([
        ...
        .then(([category1Data, category2Data, category3Data, category4Data]) => {
            // Update the dataset values in the chart
            s4_op_chart.data.datasets[0].data[0] = category1Data || 0;
            s4_op_chart.data.datasets[0].data[1] = category2Data || 0;
            s4_op_chart.data.datasets[0].data[2] = category3Data || 0;
            s4_op_chart.data.datasets[0].data[3] = category4Data || 0;
...
function updateRejBox(startDate, endDate) {
    ...
    const urlRej = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مرفوضات&siteNo=4&startDate=${startDate}&endDate=${endDate}`; // مرفوضات
    ...
    Promise.all([
        ...
        .then(([accData, rejData]) => {
            ...
            if (isNaN(percentage))
                ...
                document.getElementById("s4-rejected-per").textContent = percentage.toFixed(1) + "%"
    ...
    Promise.all([
        ...
        .then(([accData, totInData]) => {
            ...
            if (isNaN(percentage))
                ...
                document.getElementById("s4-accepted-per").textContent = percentage.toFixed(1) + "%"
...
function updateMassBox(startDate, endDate) {
    const totInURL = `http://isdom.online/dash_board/tickets/itemType/weight?itemType=مدخلات&siteNo=4&startDate=${startDate}&endDate=${endDate}`;
    const totOutURL = `http://isdom.online/dash_board/tickets/itemType/weight?itemType=مخرجات&siteNo=4&startDate=${startDate}&endDate=${endDate}`;
    const accURL = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات  تصلح للمعالجة&siteNo=4&startDate=${startDate}&endDate=${endDate}`; // مخلفات تصلح للمعالجة
    const rejURL = `http://isdom.online/dash_board/tickets/itemName/weight?itemName=مخلفات لا تصلح للمعالجة&siteNo=4&startDate=${startDate}&endDate=${endDate}`; // مخلفات لا تصلح للمعالجة
    ...
    Promise.all([
        ...
        document.getElementById("s4-mass-box").textContent = newValue.toFixed(0) + " طن";
    })
        .catch(error => {
            console.error('Error:', error);
        });
